...
interface PlayerSetupProps {
  onDone: (names: [string, string]) => void;
  onExit: () => void;
  initialNames?: string[];
}
...
export function PlayerSetup({ onDone, onExit, initialNames }: PlayerSetupProps) {
  ...
  const [names, setNames] = useState<string[]>(
    initialNames && initialNames.length >= MIN_PLAYERS ? initialNames.slice(0, MAX_PLAYERS) : ['', ''],
  );
  ...
  useEffect(() => {
    ...
    loadSavedNames(PLAYER_NAMES_KEY).then((saved) => {
      if (saved && saved.length >= MIN_PLAYERS) setNames(saved.slice(0, MAX_PLAYERS));
      didLoad.current = true;
    });
  }, []);
  ...
  function proceed() {
    onDone([names[0].trim(), names[1].trim()]);
  }
