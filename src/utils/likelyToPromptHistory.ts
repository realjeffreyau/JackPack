...
export const LIKELY_TO_PROMPT_HISTORY_KEY = 'jackpack:likely-to-recent-prompts';
...
export async function loadRecentPrompts(): Promise<string[]> {
  try {
    const raw = await AsyncStorage.getItem(LIKELY_TO_PROMPT_HISTORY_KEY);
    if (!raw) return [];
...
export async function recordUsedPrompt(prompt: string): Promise<void> {
  try {
    ...
    await AsyncStorage.setItem(LIKELY_TO_PROMPT_HISTORY_KEY, JSON.stringify(trimmed));
  } catch {}
}
